...
resource "awscc_datazone_environment_profile" "this" {
  ...
  environment_blueprint_identifier = coalesce(var.blueprint_map[each.value.environment_blueprint_identifier].environment_blueprint_id, each.value.project_identifier)
